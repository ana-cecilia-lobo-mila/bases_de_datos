...
def generate_season_sql(seasons:dict) -> str:
    lines: list[str] = [
    ...
    for s in seasons.values():
        lines.append(
            f"INSERT INTO season (name) VALUES ({val(s['name'])});"
        )
    return "\n".join(lines)
...
def generate_rating_sql(ratings: dict) -> str:
    lines: list[str] = [
    ...
    for r in ratings.values():
        lines.append(
            f"INSERT INTO rating (name) VALUES ({val(r['name'])});"
        )
    return "\n".join(lines)
...
def generate_type_sql(types: dict) -> str:
    lines: list[str] = [
    ...
    for t in types.values():
        lines.append(
            f"INSERT INTO type (name) VALUES ({val(t['name'])});"
        )
    return "\n".join(lines)
...
def generate_source_sql(sources: dict) -> str:
    lines: list[str] = [
    ...
    for s in sources.values():
        lines.append(
            f"INSERT INTO source (name) VALUES ({val(s['name'])});"
        )
    return "\n".join(lines)
...
def generate_anime_sql(animes_list: list[dict]) -> str:
    ...
    for a in animes_list:
        lines.append(
            f"INSERT INTO anime (anime_id, url, image_url, small_image_url, large_image_url, trailer_youtube_id, trailer_url, trailer_embed_url, approved, title_default, title_japanese, title_english, episodes, status, airing, aired_from, aired_to, duration, score, scored_by, rank, popularity, members, favorites, synopsis, background, year, broadcast_day, broadcast_time, broadcast_timezone, broadcast_string, season_id, rating_id, type_id, source_id)"
...
def generate_producers_sql(producers: list[dict]) -> str:
    ...
    for p in producers:
        lines.append(
            f"INSERT INTO producer (producer_id, name, url) "
...
def generate_explicit_genres_sql(explicit_genres: list[dict]) -> str:
    ...
    for e in explicit_genres:
        lines.append(
            f"INSERT INTO explicit_genre (explicit_genre_id, type, name, url) "
...
def generate_anime_studio_sql(pairs: list[tuple[int, int]]) -> str:
    ...
    for aid, sid in pairs:
        lines.append(
            f"INSERT INTO anime_studio (anime_id, studio_id) VALUES ({aid}, {sid});"
        )
    return "\n".join(lines)
...
def extract_character(characters: list[dict]) -> list[dict]:
    ...
    for c in characters:
        ...
        for d in data:
            ...
            if cid not in unique_characters:
                unique_characters[cid] = {
                    "character_id": cid,
                    "url": char["url"],
    ...
    for c in characters:
        ...
        for d in data:
            ...
            for va in d.get("voice_actors", []):
                ...
                if vid is not None:
                    pairs.add((char["mal_id"], vid))

    return [
        {"character_id": cid, "voice_actor_id": vid}
        for cid, vid in sorted(pairs)
    ]



def generate_character_voice_actor_sql(character_voice_actor: list[dict]) -> str:
    lines: list[str] = [
        "DROP TABLE IF EXISTS character_voice_actor CASCADE;",
        "",
        "CREATE TABLE character_voice_actor (",
        "    character_id INTEGER REFERENCES characters(character_id),",
        "    voice_actor_id INTEGER REFERENCES voice_actor(voice_actor_id),",
...
# Tabla principal
    ...
    dump_parts.append(generate_anime_producer_sql(anime_producers_pairs))
